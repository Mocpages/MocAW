...
public abstract class NpcAI extends EntityAIBase
{
	...
	protected final void moveToPosition(double x, double y, double z, double sqDist, double speed)
	{
		moveRetryDelay--;
		if(moveRetryDelay<=0)
		{
	...
	{
		moveToPosition(x,y,z,sqDist,1.d);
	}

	protected final void moveLongDistance(double x, double y, double z, double speed){
		...
		npc.getNavigator().tryMoveToXYZ(vec.xCoord, vec.yCoord, vec.zCoord, speed);
	}

	protected final void moveLongDistance(double x, double y, double z){
		moveLongDistance(x,y,z,1.d);
	}
}
