...
public class ItemBuildingScanner extends Item implements IItemKeyInterface, IItemClickable
{

public ItemBuildingScanner(String localizationKey)
  {
...
public void onRightClick(EntityPlayer player, ItemStack stack)
  {
	ItemBuildingSettings.getSettingsFor(stack, scanSettings);
  if(player.isSneaking())
    {
    scanSettings.clearSettings();
    ItemBuildingSettings.setSettingsFor(stack, scanSettings);
    }
  }
...
public void onKeyAction(EntityPlayer player, ItemStack stack, ItemKey key){  
  if(key == ItemKey.KEY_0) {//Z
	  BlockPosition hit = BlockTools.getBlockClickedOn(player, player.worldObj, player.isSneaking(), 64);
	  if(hit==null){return;}
	  ItemBuildingSettings.getSettingsFor(stack, scanSettings);
